...
def main():
    ...
    cols = ['climate_scenario', 'edge_id', 'hazard_type', 'max_depth', 'min_depth',
    ...
    for m in range(len(modes)):
        if modes[m] != 'road':
            ...
            index_cols = ['edge_id', 'hazard_type', 'model', 
                            ...
                            'road_type', 'road_cond', 'width', 'edge_length']
            sel_cols = ['edge_id','road_type', 'road_cond','width', 'length']
        ...
        G_df = pd.read_csv(os.path.join(network_data_excel,
                                          '{}_edges.csv'.format(modes[m])), encoding='utf-8')
        G_df = G_df[sel_cols]
