...
def main():
    ...
    for dur in duration:
        flow_file_path = os.path.join(config['paths']['output'], 
                                    'adaptation_results',
                                    'combined_climate',
                                   'output_adaptation_bridge_{}_days_max_{}_growth_disruption_fixed_parameters.csv'.format(dur,
                                                                                        growth_rate))
        ...
        change_df.to_csv(os.path.join(config['paths']['output'],
            'network_stats',
            'national_bridge_max_bc_ratios_climate_change_{}_days.csv'.format(dur)
            ), index=False
        )
        ...
        for sc in scenarios:
            ...
            output_file = os.path.join(config['paths']['figures'],
                                       'national-bridges-{}-{}-bc-ratios-change-percentage-{}-days.png'.format(climate_scenario.replace('-',' ').title(),
                                                                                                                year,dur))
        ...
        #         config['paths']['figures'], 'national-bridges-{}-{}-days.png'.format(adapt_set[c]['column'],dur))
        #     save_fig(output_file)
        #     plt.close()
        ...
        #             'national-bridges-{}-{}-million-investment-{}-days.png'.format(adapt_set[c]['column'],cost,dur))
        #         save_fig(output_file)
        #         plt.close()
